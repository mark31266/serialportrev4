...
function updatefunction() 
{
 ...
 db.collection("auditlog").doc(date2.innerText + " " + clock1.innerText).set(
  ...
  db.collection("patientvalues").doc(username).collection("DATE").doc(date1).collection("SID").doc(sid1).collection("DATA").get().then(function(querySnapshot) 
  {
...
function deletefunction() 
{
  ...
  var sid1 = document.getElementById("sidselect").value; 
  db.collection("patientvalues").doc(username).collection("DATE").doc(date1).collection("SID").doc(sid1).delete().then(() => {
    window.location.reload(); 
}).catch((error) => {
    alert("Error removing document: ", error);
});
